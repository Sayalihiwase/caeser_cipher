...
userDecodeInput.oninput = () => {
    var EncodeKey = document.getElementById('cipherEncodeKey').value;
    if(EncodeKey<=25 && EncodeKey>=1){
	caesarCipherDecode(userDecodeInput.value, cipherDecodeKey.value);
    }
}

cipherDecodeKey.oninput = () => {
    var DecodeKey = document.getElementById('cipherDecodeKey').value;
    if(DecodeKey<=25 && DecodeKey>=1){
	caesarCipherDecode(userDecodeInput.value, cipherDecodeKey.value);
    }
}